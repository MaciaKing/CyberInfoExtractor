...
func (fr *FileReader) ReadFileFromChan(filepath string, startLineToRead int, totalsLineToRead int, linesReadString chan string) (int, error) {
	file, err := os.Open(filepath)
	if err != nil {
		return -1, err
	}
...
func (fr *FileReader) TotalLineCounter(filepath string) (int, error) {
	file, err := os.Open(filepath)
	if err != nil {
		return -1, err
	}
